    ...
    if user_data.role in ["Staff", "SPV"] and user_data.division:
        ...
        if manager:
            await create_notification(
                ...
                message=f"{user_data.username} ({user_data.role} - {user_data.division}) has registered and needs approval",
...
async def get_pending_accounts(current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] == "Manager":
        ...
        # NEW: VP only approves Manager accounts
        query["role"] = "Manager"
...
async def review_account(action_data: AccountApprovalAction, current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] == "Manager":
        ...
        # NEW: VP can only review Manager accounts
        if user.get("role") != "Manager":
            raise HTTPException(status_code=403, detail="VP can only review Manager accounts")
