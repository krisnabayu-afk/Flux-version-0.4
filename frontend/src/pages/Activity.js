...
const Activity = () => {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'Pending': { color: 'bg-gray-800 text-gray-300 border-gray-700', icon: Clock },
      'In Progress': { color: 'bg-gray-700/50 text-gray-200 border-gray-600', icon: Play },
      'Finished': { color: 'bg-green-900/30 text-green-300 border-green-800', icon: CheckCircle },
      'Cancelled': { color: 'bg-red-900/30 text-red-300 border-red-800', icon: XCircle },
      'On Hold': { color: 'bg-yellow-900/30 text-yellow-300 border-yellow-800', icon: Pause }
    };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-300">{moment().format('MMMM DD, YYYY')} - Record your daily task execution</p>
        </div>
      </div>

      <div className="space-y-4">
        <h2 className="text-xl font-bold text-white">Your Schedules for Today</h2>
        {loading && schedules.length === 0 ? (
          <div className="text-center py-12"><p className="text-gray-400">Loading schedules...</p></div>
        ) : schedules.length === 0 ? (
          <Card><CardContent className="py-12"><p className="text-center text-gray-400">No schedules assigned for today. Enjoy your day!</p></CardContent></Card>
        ) : (
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        {schedule.description && (<p className="text-sm text-gray-300 mt-1">{schedule.description}</p>)}
                    ...
                    {schedule.latest_activity && (
                      <div className="mt-2 p-3 bg-slate-900/50 rounded-lg border border-slate-700 space-y-2">
                        <p className="text-xs font-semibold text-gray-400">Latest Activity:</p>
                        <p className="text-xs text-gray-300">{schedule.latest_activity.action_type.toUpperCase()} - {moment(schedule.latest_activity.created_at).format('MMM DD, HH:mm')}</p>
                        {schedule.latest_activity.notes && (<p className="text-xs text-gray-300/80">Notes: {schedule.latest_activity.notes}</p>)}
                        {schedule.latest_activity.reason && (<p className="text-xs text-gray-300/80">Reason: {schedule.latest_activity.reason}</p>)}
                        ...
                        {schedule.latest_activity.progress_updates && schedule.latest_activity.progress_updates.length > 0 && (
                          <div className="mt-2 pt-2 border-t border-gray-600">
                            <p className="text-xs font-semibold text-gray-200 mb-2 flex items-center"><MessageSquarePlus size={12} className="mr-1" />Progress Updates:</p>
                            <div className="space-y-1 max-h-40 overflow-y-auto">
                              {schedule.latest_activity.progress_updates.map((update, idx) => (
                                <div key={idx} className="text-xs text-gray-300 bg-gray-800/80 p-2 rounded border border-gray-700">
                                  <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-[10px] text-gray-400 ml-2 whitespace-nowrap flex flex-col items-end">
                                      ...
                                      {update.latitude && update.longitude && (
                                        <a
                                          ...
                                          target="_blank"
                                          rel="noopener noreferrer"
                                          className="flex items-center text-gray-400 hover:text-gray-200 mt-1"
                                          title="View Location"
                                        >
                        ...
                        {schedule.latest_activity.id && schedule.activity_status === 'In Progress' && (
                          <div className="mt-2 pt-2 border-t border-gray-700">
                            <Label className="text-xs font-semibold text-gray-200 mb-1 block">Add Live Update:</Label>
