...
import { ClipboardCheck, Play, CheckCircle, Pause, XCircle, Clock, MessageSquarePlus, Send, RotateCcw, Image as ImageIcon } from 'lucide-react';
                              ...
                              {schedule.latest_activity.progress_updates.map((update, idx) => (
                                <div key={idx} className="text-xs text-slate-600 bg-white p-2 rounded border border-slate-200">
                                  <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-[10px] text-slate-400 ml-2 whitespace-nowrap">{moment(update.timestamp).format('HH:mm')}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
