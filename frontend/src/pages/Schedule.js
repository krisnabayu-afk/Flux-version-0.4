...
import { Calendar as CalendarComponent } from '../components/ui/calendar'; // Renaming to avoid conflict with big-calendar
        ...
        {showDatePicker && (
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-slate-900 border border-slate-700 rounded-lg shadow-xl p-4 z-50 min-w-[300px]">
            <div className="space-y-3">
              <div>
                <Label className="text-xs font-semibold text-slate-300 mb-1 block">Month</Label>
              ...
              <div>
                <Label className="text-xs font-semibold text-slate-300 mb-1 block">Year</Label>
...
const Schedule = () => {
  ...
  return (
    <div className="space-y-6" data-testid="schedule-page">
          ...
          {canEdit && (
            <>
              ...
              <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild>
                  <Button className="bg-gray-600 hover:bg-gray-700" data-testid="create-schedule-button">
                ...
                <DialogContent className="max-w-xl max-h-[85vh] overflow-y-auto w-full" data-testid="schedule-dialog">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            // Basic validation to prevent > 4 digit year year
                            // Allowed: numbers, dashes, spaces, colons, 'T'
                            if (/^[\d\-\s:T]*$/.test(val)) {
                              // Check year part length if it exists
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <CalendarComponent
                              ...
                              onSelect={(date) => {
                                if (!date) return;
                                // Preserve time if exists, otherwise default to current time
                                const currentStr = formData.start_date || '';
                                const timePart = currentStr.includes('T') ? currentStr.split('T')[1] : (currentStr.includes(' ') ? currentStr.split(' ')[1] : moment().format('HH:mm'));

                                const newDateStr = moment(date).format('YYYY-MM-DD');
                                const finalStr = `${newDateStr} ${timePart || '09:00'}`;
                                setFormData({ ...formData, start_date: finalStr });
                              }}
                              initialFocus
                            />
                          </PopoverContent>
                    ...
                    <div className="flex justify-end space-x-2">
                      ...
                      <Button type="submit" className="bg-gray-600 hover:bg-gray-700" data-testid="submit-schedule-button">
                        Create Schedule
                      </Button>
                    </div>
                  </form>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-slate-900/50 rounded-lg border border-slate-700 shadow-sm gap-4">
        <div className="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-3 flex-1 w-full">
          <Label className="text-sm font-semibold text-slate-300 whitespace-nowrap">Filter by Staff:</Label>
          <StaffCombobox
            users={users}
            value={filterUserId}
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-3 flex-1 w-full">
          <Label className="text-sm font-semibold text-slate-300 whitespace-nowrap">Filter by Division:</Label>
      ...
      {/* Legend */}
      <div className="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4 p-4 bg-slate-900/50 rounded-lg border border-slate-700 shadow-sm">
        <span className="text-sm font-semibold text-slate-300">Divisions:</span>
      ...
      <div className="bg-slate-900/50 rounded-xl p-6 shadow-lg border border-slate-700">
        ...
        <div className="space-y-3">
          {(() => {
            ...
            return todaySchedules.map(schedule => (
              <div key={schedule.id} className="flex items-center justify-between p-4 border border-slate-700 rounded-lg hover:bg-slate-800 transition-colors">
                <div className="flex-1">
                  <h3 className="font-semibold text-slate-100">{schedule.title}</h3>
          ...
          {selectedScheduleForActivity && (
            <div className="space-y-4">
              <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                <h3 className="font-semibold text-lg text-white">{selectedScheduleForActivity.title}</h3>
                <p className="text-sm text-slate-400 mb-2">{selectedScheduleForActivity.user_name} ({selectedScheduleForActivity.division})</p>
              ...
              {loadingActivity ? (
                ...
                <div className="space-y-4">
                  ...
                  {activityData.reason && (
                    <div className="bg-red-900/20 p-3 rounded text-sm text-red-200 border border-red-800/50">
                    ...
                    <div className="border-t pt-3">
                      ...
                      <div className="space-y-2 max-h-60 overflow-y-auto">
                        {activityData.progress_updates.map((update, idx) => (
                          <div key={idx} className="text-sm bg-slate-800/50 border border-slate-700 p-2 rounded shadow-sm">
                              ...
                              <span className="text-xs text-slate-400 whitespace-nowrap ml-2 flex flex-col items-end">
                                ...
                                {update.latitude && update.longitude && (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center text-gray-400 hover:text-gray-300 mt-1"
                                    title="View Location"
                                  >
